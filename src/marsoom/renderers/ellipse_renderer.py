...
class EllipseRenderer:
    def __init__(self, num_segments: int = 20):
        ...
        self.num_floats_per_element = [3, 3, 3, 1]  # position, color, cov2d, opacity
    ...
    def _copy(
        ...
        with torch.no_grad():
            ...
            for num_floats, tensor in zip(
                self.num_floats_per_element, [positions, colors, cov2D, opacity]
            ):
    ...
    def update(
        ...
        self._copy(positions=positions, colors=colors, cov2D=cov2D, opacity=opacity)
