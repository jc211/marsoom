...
class VertexDomain:
    ...
    buffer_attributes: list[tuple[BufferObject, Attribute]]
    vao: VertexArray
    attribute_names: dict[str, Attribute]
    attrib_name_buffers: dict[str, BufferObject]
    ...
    def __init__(self, attribute_meta: dict[str, dict[str, Any]]) -> None:  # noqa: D107
        ...
        for name, meta in attribute_meta.items():
            ...
            # Create buffer:
            size = count*ctypes.sizeof(attribute.c_type)
            self.attrib_name_buffers[name] = buffer = BufferObject(size)
    ...
    def get_buffer(self, name: str) -> BufferObject:
    ...
    def update_buffer(self, name: str, data: np.ndarray) -> None:
        buffer = self.get_buffer(name)
        if buffer.size != data.nbytes:
            print(f"Resizing buffer {name} from {buffer.size} to {data.nbytes}")
            buffer.resize(data.nbytes)
...
def vertex_list_create(
        program: ShaderProgram, 
        indices: Sequence[int] | None = None,
        instances: Sequence[str] | None = None,
        **data: Any) -> VertexDomain:
    ...
    if indexed:
        domain = IndexedVertexDomain(attributes, indices)
    else:
        domain = VertexDomain(attributes)
